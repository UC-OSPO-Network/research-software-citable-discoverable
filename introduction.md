...
This isn't hypothetical, even for critical infrastructure. Before 2014, OpenSSL secured something like a sixth of the web's encrypted traffic and ran on about $2,000 a year in donations, with exactly one person paid to maintain it, plus a few volunteers. Then Heartbleed happened, and it became public knowledge just how much of the internet depended on that one person's continued availability. Read more: [Core Infrastructure Initiative](https://en.wikipedia.org/wiki/Core_Infrastructure_Initiative).

Depositing a release into an archive like Zenodo is the software equivalent of accessioning: it moves the object from a personal account to a preservation system that outlives the individual.
