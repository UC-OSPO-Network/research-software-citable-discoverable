...
The [`06-optional-pixi` reference branch][branch-pixi] builds on [`05-add-metadata`][branch-metadata], so it shows pixi added to an already-finished project. Have learners view it in the browser for the target `pixi.toml` and lockfile, then work along in their local clone. Nothing in the main project chain depends on pixi, so skipping this episode leaves no loose ends in anyone's repository.
